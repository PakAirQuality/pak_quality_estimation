"""
Feature Store Paths Configuration
=================================

Defines where the feature store lives and how partitions are laid out.

Storage Layout:
    {base_path}/station/{stage}/date={YYYY-MM-DD}/part-000.parquet
...
Stages:
    ...
    - geostatic: Elevation, distance-to-features (rarely changes)
"""
...
# Valid stage names
VALID_STAGES = {"met", "aod", "tropomi", "geostatic", "master"}


@dataclass
class StoreConfig:
    """
    Configuration for feature store paths.
    ...
    @classmethod
    def local(cls, base_path: str = "feature_store") -> "StoreConfig":
    ...
    def stage_prefix(self, stage: str) -> str:
        ...
        Args:
            stage: Feature stage (met, aod, tropomi, geostatic)
    ...
    def list_partitions(self, stage: str) -> List[str]:
        ...
        if self.is_gcs:
            # Use gcsfs or gsutil to list
            try:
                import gcsfs
                fs = gcsfs.GCSFileSystem()
                # List directories under the stage prefix
...
_default_config: Optional[StoreConfig] = None


def get_default_config() -> StoreConfig:
    """Get the default store configuration."""
    global _default_config
    if _default_config is None:
        # Check environment variable for base path
        base_path = os.environ.get("FEATURE_STORE_PATH", "feature_store")
        _default_config = StoreConfig(base_path=base_path)
    return _default_config


def set_default_config(config: StoreConfig) -> None:
