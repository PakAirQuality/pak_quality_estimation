...
# -----------------------------
# AQI colors + PM2.5 breakpoints (24h), US EPA updated effective May 6, 2024
# Good: 0–9.0
# Moderate: 9.1–35.4
# USG: 35.5–55.4
...
# Hazardous: 225.5+
# -----------------------------
AQI_PM25_BREAKS = np.array([0.0, 9.0, 35.4, 55.4, 125.4, 225.4, 500.4], dtype=np.float32)
...
# Standard AQI category colors: green, yellow, orange, red, purple, maroon
AQI_COLORS = [
    "#00e400",
    "#ffff00",
    "#ff7e00",
...
class PM25VideoGenerator:
    """Generate animated videos of PM2.5 predictions over Pakistan"""

    def __init__(self, predictor: PakistanPM25Predictor):
        self.predictor = predictor
        self.predictions_cache: Dict[str, np.ndarray] = {}

        # Discrete AQI colormap (category colors) + PM2.5 breakpoints
        self.cmap = ListedColormap(AQI_COLORS, name="pm25_aqi")
        self.cmap.set_bad((0, 0, 0, 0))  # transparent NaNs
        self.norm = BoundaryNorm(AQI_PM25_BREAKS, ncolors=self.cmap.N, clip=True)

        # for convenience (used in colorbar + stats)
        self._aqi_breaks = AQI_PM25_BREAKS
        self._aqi_labels = AQI_LABELS

        # derived once from lat/lon grids
        self._grid_tf: Optional[Dict[str, object]] = None

        print("[video] PM2.5 Video Generator initialized (AQI categories)")
    ...
    # AQI stats text
    # ----------------------------
    def _aqi_stats_box(self, vec: np.ndarray) -> str:
        ...
        # Category index per point using PM2.5 breaks
        # right=True => bins[i-1] < x <= bins[i]
        cat = np.digitize(v, self._aqi_breaks[1:], right=True)
        cat = np.minimum(cat, len(self._aqi_labels) - 1)

        counts = [int(np.sum(cat == i)) for i in range(len(self._aqi_labels))]
        pcts = [c / total * 100.0 for c in counts]

        # "worst" category present (quick sanity)
        worst_i = int(np.max(cat))
        worst_label = self._aqi_labels[worst_i]

        # Compact multi-line box
        lines = [
            f"PM2.5 µg/m³  Mean {mu:.1f} | Median {med:.1f} | P95 {p95:.1f}",
            f"Min {mn:.1f} | Max {mx:.1f} | Std {std:.1f} | Worst: {worst_label}",
            "AQI Categories (% of grid):",
            "  " + " | ".join([f"{lab} {p:.1f}%" for lab, p in zip(self._aqi_labels, pcts)]),
        ]
        return "\n".join(lines)
    ...
    def create_static_map(
        ...
        # Set up dynamic normalization based on this day's data
        self._setup_dynamic_norm(predictions)
        ...
        ax.text(
            0.02,
            0.98,
            self._aqi_stats_box(predictions),
    ...
    def create_animation(
        ...
        """Create animated video of PM2.5 predictions with turbo colormap (not inverted; robust; clipped)."""
        ...
        mid = (self._aqi_breaks[:-1] + self._aqi_breaks[1:]) / 2.0
        cbar.set_ticks(mid)

        range_labels = []
        for i, lab in enumerate(self._aqi_labels):
            lo = self._aqi_breaks[i]
            hi = self._aqi_breaks[i + 1]
            if i == len(self._aqi_labels) - 1:
                range_labels.append(f"{lab}\n(≥ {lo:.1f})")
            else:
                range_labels.append(f"{lab}\n({lo:.1f}–{hi:.1f})")

        cbar.ax.set_yticklabels(range_labels)
        cbar.set_label("PM2.5 (µg/m³) — AQI categories", fontsize=12, fontweight="bold")
        ...
        def animate_frame(frame_idx: int):
            ...
            stats_text.set_text(self._aqi_stats_box(vec))
        ...
        if not animation.writers.is_available("ffmpeg"):
            ...
            raise RuntimeError(
                ...
                "You installed ffmpeg, but matplotlib can't see it. "
...
def ensure_store_exists(
    ...
    tropomi_dir = datasets_dir / "tropomi"
...
def main():
    parser = argparse.ArgumentParser(description="Create PM2.5 video animation for Pakistan (TRUE features)")
    parser.add_argument("--start_date", required=True, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end_date", required=True, help="End date (YYYY-MM-DD)")

    parser.add_argument("--model", default="best_model_weight/production_tuned_model_20251228_203856.joblib")
    parser.add_argument("--master_feature_dir", default="../inference")
    parser.add_argument("--resolution", type=float, default=0.1)
    parser.add_argument("--history_days", type=int, default=21)
    parser.add_argument("--tropomi_window", type=int, default=5, help="Odd window size for TROPOMI (5 or 7 recommended for distinct multiscale features)")
    ...
    if use_store:
        ...
        video_gen = PM25VideoGenerator(predictor)
